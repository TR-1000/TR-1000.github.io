...
const getGames = () => {
  ...
  for (game of libraryArray) {
    ...
    $("<img>").attr("src", `https://steamcdn-a.akamaihd.net/steam/apps/${game.appid}/header.jpg?`).appendTo($li)
  }
}
...
const randomUnplayedGame = () => {
  ...
  $("<img>").attr("src",`https://steamcdn-a.akamaihd.net/steam/apps/${randomGame.appid}/header.jpg?`)
  .appendTo($("main")).on("click", () => {
    $("<ul>").text("Playlist").attr("id","playlist").appendTo($("main"));
    $("<li>").appendTo($("#playlist"));
    $("<a>").attr({
      "href":`https://store.steampowered.com/${randomGame.appid}/`,
      "target":"_black"}).text(`${randomGame.name}`).appendTo($("li"));
  })
